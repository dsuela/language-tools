...
final class ServerVersion
{
    ...
    public function __construct(?string $version = null)
    {
        if (null === $version) {
            ...
            if (false === $version) {
                throw new \RuntimeException('The Symfony Language Tools version file is unavailable.');
            }
        }

        $version = trim($version);
        if ('' === $version || 1 !== preg_match('/^(?:v?\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?|dev)$/', $version)) {
            throw new \InvalidArgumentException(\sprintf('Invalid Symfony Language Tools version "%s".', $version));
        }
